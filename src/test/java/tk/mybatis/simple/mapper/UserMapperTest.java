...
public class UserMapperTest extends BaseMapperTest {
    ...
    public void testSelectAll2() {
        ...
        try {
            UserMapper userMapper = sqlSession.getMapper(UserMapper.class);
            //调用 selectAll 方法查询所有用户
            List<Map<String, Object>> userList = userMapper.selectAll2();
            //结果不为空
            Assert.assertNotNull(userList);
            //用户数量大于 0 个
            Assert.assertTrue(userList.size() > 0);
    ...
    @Test
    public void testSelectAll_my_proxy() {
        SqlSession sqlSession = getSqlSession();
        try {
            MyMapperProxy mapperProxy = new MyMapperProxy(UserMapper.class, sqlSession);
            UserMapper userMapper = (UserMapper) Proxy.newProxyInstance(Thread.currentThread().getContextClassLoader(), new Class[]{UserMapper.class}, mapperProxy);
...
//            sqlSession.commit();
    ...
    public void testSelectByIdList2() {
        ...
        try {
            ...
            idList.add(1001L);
            //业务逻辑中必须校验 idList.size() > 0
            //<editor-fold desc="Java中List转换为数组">
            Long[] idArray = new Long[idList.size()];
            Long[] array = idList.toArray(idArray);
            //</editor-fold>
            List<SysUser> userList = userMapper.selectByIdList2(array);
            Assert.assertEquals(2, userList.size());
        } finally {
            //不要忘记关闭 sqlSession
            sqlSession.close();
    ...
    public void testUpdateByMap() {
        ...
        try {
            ...
            map.put("user_password", "sasasasa");
            //更新数据
